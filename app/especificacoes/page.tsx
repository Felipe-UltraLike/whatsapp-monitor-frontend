...
import { Bug, Sparkles, Zap, Search, ClipboardList, RefreshCw, Clipboard } from 'lucide-react';
...
const SPEC_TYPE_ICON: Record<string, React.ElementType> = {
  bug_fix:      Bug,
  feature:      Sparkles,
  improvement:  Zap,
  investigation:Search,
};
const PRIORITY_BADGE: Record<string, string> = { critical: 'badge-red', high: 'badge-orange', medium: 'badge-yellow', low: 'badge-green' };
const PRIORITY_LABEL: Record<string, string> = { critical: 'Crítica', high: 'Alta', medium: 'Média', low: 'Baixa' };
const STATUS_LABEL: Record<string, string> = { pending: 'Pendente', assigned: 'Atribuído', in_progress: 'Em andamento', done: 'Concluído' };
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
              {loading ? (
                ...
                <div className="empty-state" style={{ padding: '40px 20px' }}>
                  <ClipboardList size={36} color="var(--text2)" />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {filtered.map(spec => {
                    const TypeIcon = SPEC_TYPE_ICON[spec.spec_type] || ClipboardList;
                    return (
                      <div
                        key={spec.id}
                        onClick={() => setSelected(spec)}
                        className="spec-card"
                        style={{ cursor: 'pointer', borderColor: selected?.id === spec.id ? 'var(--primary)' : 'var(--border)', opacity: spec.status === 'done' ? 0.65 : 1 }}
                      >
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-xs text-muted" style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                            <TypeIcon size={11} /> {SPEC_TYPE_LABEL[spec.spec_type]}
                          </span>
                        </div>
          ...
          <div className="split-detail" style={{ flex: 1, overflowY: 'auto', padding: 28 }}>
            {!selected ? (
              <div className="empty-state">
                <ClipboardList size={48} color="var(--text2)" />
              ...
              <>
                  ...
                  <div style={{ flex: 1 }}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="badge badge-gray" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                        {(() => { const I = SPEC_TYPE_ICON[selected.spec_type]; return I ? <I size={11} /> : null; })()}
                        {SPEC_TYPE_LABEL[selected.spec_type]}
                      </span>
                      <span className="badge badge-gray" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                        <Zap size={11} /> {COMPLEXITY_LABEL[selected.estimated_complexity]}
                      </span>
                    </div>
                    ...
                    <div className="text-sm text-muted" style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                      ...
                      {selected.system_name && <span>{selected.system_name}</span>}
                      <span>{new Date(selected.created_at).toLocaleString('pt-BR')}</span>
                    </div>
                  </div>
                  <button className="btn btn-ghost btn-sm" onClick={() => copyForDev(selected)} style={{ flexShrink: 0, marginLeft: 16, display: 'flex', alignItems: 'center', gap: 5 }}>
                    <Clipboard size={13} /> Copiar para Dev
                  </button>
                </div>
                ...
                <div className="card mb-6" style={{ padding: '16px 20px' }}>
                  <div className="flex gap-4" style={{ flexWrap: 'wrap' }}>
                    <div>
                      ...
                      <select value={selected.status} onChange={e => updateSpec(selected.id, { status: e.target.value })} style={{ width: 'auto' }}>
                        <option value="pending">Pendente</option>
                        <option value="assigned">Atribuído</option>
                        <option value="in_progress">Em andamento</option>
                        <option value="done">Concluído</option>
                    ...
                    <div>
                      ...
                      <select value={selected.assigned_dev || ''} onChange={e => updateSpec(selected.id, { assigned_dev: e.target.value, status: e.target.value ? 'assigned' : 'pending' })} style={{ width: 'auto' }}>
                ...
                {selected.original_message && (
                  <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 18px', marginBottom: 20 }}>
                    ...
                    <p className="text-sm" style={{ fontStyle: 'italic', lineHeight: 1.7 }}>&ldquo;{selected.original_message}&rdquo;</p>
                  </div>
                )}
